...
export const MyComposition: React.FC = () => {
  ...
  const logoScale = interpolate(logoPop, [0, 1], [0.28, 1.04], clamp);
  ...
  const pathReveal = interpolate(frame, [0.25 * fps, 1.35 * fps], [0, 1], {
    ...clamp,
    easing: easeOut,
  });
  const pathSplit = interpolate(frame, [1.05 * fps, 1.95 * fps], [0, 1], {
    ...clamp,
    easing: easeOut,
  });
  const pathOpacity = interpolate(frame, [0.15 * fps, 0.45 * fps, 2.0 * fps, 2.55 * fps], [0, 1, 1, 0], {
    ...clamp,
    easing: easeOut,
  });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path
          d={`M ${width / 2} ${height + 90} C ${width / 2} ${height * 0.72}, ${width / 2} ${height * 0.5}, ${width / 2} ${height * 0.32}`}
          stroke="url(#path-gradient)"
          strokeWidth={7}
          strokeLinecap="round"
          fill="none"
          strokeDasharray={720}
          strokeDashoffset={720 - 720 * pathReveal}
          filter="drop-shadow(0 0 18px rgba(57,216,182,0.42))"
        />
        <path
          d={`M ${width / 2} ${height * 0.32} C ${width * 0.43} ${height * 0.26}, ${width * 0.35} ${height * 0.26}, ${width * 0.26} ${height * 0.22}`}
          stroke="url(#path-gradient)"
          strokeWidth={6}
          strokeLinecap="round"
          fill="none"
          strokeDasharray={420}
          strokeDashoffset={420 - 420 * pathSplit}
          filter="drop-shadow(0 0 18px rgba(57,216,182,0.35))"
        />
        <path
          d={`M ${width / 2} ${height * 0.32} C ${width * 0.57} ${height * 0.26}, ${width * 0.65} ${height * 0.26}, ${width * 0.74} ${height * 0.22}`}
          stroke="url(#path-gradient)"
          strokeWidth={6}
          strokeLinecap="round"
          fill="none"
          strokeDasharray={420}
          strokeDashoffset={420 - 420 * pathSplit}
          filter="drop-shadow(0 0 18px rgba(38,99,255,0.35))"
        />
      </svg>
